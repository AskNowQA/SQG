...
class Trainer(object):
    ...
    def train(self, dataset):
        ...
        for idx in tqdm(range(len(dataset)), desc='Training epoch ' + str(self.epoch + 1) + ''):
            ...
            target = Var(torch.FloatTensor([label]))  # map_label_to_target(label, dataset.num_classes))
            if self.args.cuda:
    ...
    def test(self, dataset):
        ...
        for idx in tqdm(range(len(dataset)), desc='Testing epoch  ' + str(self.epoch) + ''):
            ...
            target = Var(torch.FloatTensor([label]),
                         volatile=True)  # map_label_to_target(label, dataset.num_classes), volatile=True)
            if self.args.cuda:
                linput, rinput = linput.cuda(), rinput.cuda()
                target = target.cuda()
            output = self.model(linput, rinput)
            ...
            predictions[idx] = output[0]  # torch.dot(indices, torch.exp(output))
